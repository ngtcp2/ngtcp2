...
if [ "$ROLE" == "client" ]; then
    ...
    CLIENT_ARGS="$SERVER 443 --download /downloads -s -q --no-quic-dump --no-http-dump --exit-on-all-streams-close --qlog-dir $QLOGDIR --cc bbr --initial-rtt 100ms"
    ...
    SERVER_ARGS="/certs/priv.key /certs/cert.pem -s -d /www -q --qlog-dir $QLOGDIR --cc bbr --initial-rtt 100ms"
    case "$TESTCASE" in
        "retry")
